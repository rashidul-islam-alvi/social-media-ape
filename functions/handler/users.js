...
exports.signUp = (req, res) => {
  ...
  const { valid, errors } = validationSignUp(newUser);
...
exports.logIn = (req, res) => {
  ...
  const { errors, valid } = validationLogIn(user);
...
exports.getOwnUserDetails = (req, res) => {
  ...
  db.doc(`/users/${req.user.handle}`)
    ...
    .then((doc) => {
      if (doc.exists) {
        ownUserDetails.Credentials = doc.data();
    ...
    .then((data) => {
      ownUserDetails.likes = [];
      data
        .forEach((doc) => {
          ownUserDetails.likes.push(doc.data());
        })
